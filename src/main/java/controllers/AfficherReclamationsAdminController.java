...
public class AfficherReclamationsAdminController {
    ...
    private void mettreEnCours(Reclamation reclamation) {
        try {
            // Check if the reclamation is already "EN_COURS"
            if (reclamation.getStatus() == Status.EN_COURS) {
                showAlert(Alert.AlertType.INFORMATION, "Information", "Cette réclamation est déjà en cours de traitement.");
                return; // Exit the method if the status is already "EN_COURS"
            }
            if (reclamation.getStatus() == Status.RESOLU) {
                showAlert(Alert.AlertType.INFORMATION, "Information", "Cette réclamation est déjà résolu.");
                return; // Exit the method if the status is already "EN_COURS"
            }

            // Update the status to "EN_COURS" in the database
            ...
            showAlert(Alert.AlertType.INFORMATION, "Succès", "La réclamation a été mise en cours de traitement.");

        } catch (SQLException e) {
            e.printStackTrace();
            showAlert(Alert.AlertType.ERROR, "Erreur", "Une erreur s'est produite lors de la mise à jour du statut.");
        } catch (Exception e) {
